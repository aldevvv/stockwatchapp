...
export const createListing = async (req, res) => {
    try {
        ...
        const newListing = {
            id: newListingRef.key, userId, namaToko: profileData.namaToko,
            kontakWhatsApp: profileData.nomorWhatsAppNotifikasi, fotoProfilUrl: profileData.fotoProfilUrl || '',
            itemId, namaBarang: itemData.namaBarang, jumlahDitawarkan: Number(jumlahDitawarkan),
            satuan: itemData.satuan, hargaPerUnit: Number(hargaPerUnit),
            catatan: catatan || '', status: 'TERSEDIA', createdAt: Date.now(),
        };
        await newListingRef.set(newListing);
        
        const statsRef = db.ref(`userStats/${userId}/listingCount`);
        await statsRef.transaction((currentCount) => (currentCount || 0) + 1);
        const unlockedAchievements = await checkAndAwardAchievements(userId, 'LISTING_COUNT');

        res.status(201).json({ message: 'Barang berhasil dilisting!', data: newListing, unlockedAchievements });
    } catch (error) {
...
export const getAllListings = async (req, res) => {
    try {
        ...
        if (!snapshot.exists()) {
            return res.status(200).json({ message: 'Belum ada stok yang dijual.', data: [] });
        }
...
export const getMyListings = async (req, res) => {
    try {
        ...
        const snapshot = await listingsRef.once('value');
        if (!snapshot.exists()) {
            return res.status(200).json({ message: 'Anda belum memiliki listing.', data: [] });
        }
